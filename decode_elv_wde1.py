...
# install with pypy -m pip install paho-mqtt
# import context  # Ensures paho is in PYTHONPATH
import paho.mqtt.publish as publish


class decoder(object):
    def __init__(self, mqttparams):
        ...
        print(self.mqttparams['hostname'])
    ...
    def process(self, value):
        ...
        if self.clipped % 10000 == 1:
            logging.error(
                "Clipped signal detected, you should reduce gain of receiver!")
    ...
    def bitval(self):
        ...
        logging.debug(
            "bitval: a={0} m={1} e={2} val={3}".format(aa, ma, ea, val))
        return val
    ...
    def decode(self):
        sensor_types = ('Thermo', 'Thermo/Hygro', 'Rain(?)', 'Wind(?)',
                        'Thermo/Hygro/Baro', 'Luminance(?)', 'Pyrano(?)', 'Kombi')
        ...
        if sum_read != sum:
            logging.warn(
                "Sum read is {0} but computed is {1}".format(sum_read, sum))
            return
        ...
        jsonout = json.dumps(decoder_out)
        publish.single(self.mqttparams['topic'], jsonout,
                       hostname=self.mqttparams['hostname'], auth={'username': self.mqttparams['user'], 'password': self.mqttparams['password']})
...
def main():
    parser = argparse.ArgumentParser(
        description='Decoder for weather data of sensors from ELV received with RTL SDR.')
    parser.add_argument('--log', type=str, default='WARN',
                        help='Log level: DEBUG|INFO|WARN|ERROR. Default: WARN')
    parser.add_argument('--mqtttopic', type=str, default='rtl/sensor/weather',
                        help='Topic for MQTT')
    parser.add_argument('--mqtthost', type=str, default='',
                        help='IP/Hostname of MQTT-Broker')
    parser.add_argument('--mqttuser', type=str,
                        default='', help='User for MQTT')
    parser.add_argument('--mqttpassword', type=str,
                        default='', help='Password for MQTT')
    parser.add_argument('inputfile', type=str, nargs=1,
                        help="Input file name. Expects a raw file with signed 16-bit samples in platform default byte order and 30 kHz sample rate. Use '-' to read from stdin. Example: rtl_fm -M -f 868.35M -s 30k | ./decode_elv_wde1.py -")
